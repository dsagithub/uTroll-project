...
//		$('<strong> Eres el Troll del grupo! </strong>').appendTo($('#group_state'));
}}
	...
	if (gid == 0) {
		...
		$.ajax({
			...
			var group = data;
				getTrollMode();
				$("#group_details").text('');
				$('<strong> GroupID: </strong> ' + group.groupid + '<br>').appendTo($('#group_details'));
				
				$('<h4><strong> Creado por  ' + group.creator + '</strong><br> </h4>').appendTo($('#group_details'));
				
				$('<h4><strong> Estado: </strong> ' + group.state + '<br> </h4>').appendTo($('#group_details'));
				
				
				$('<strong><h1>' + group.price + '</h1></strong>').appendTo($('#group_details2'));
				$("#group_details2").attr("style", " text-align:center;font-size:50");
				$('#group_title').text(group.groupname);

				$('<strong> Tienes: ' + getCookie('points') + '</strong> puntos<br>').appendTo($('#group_state'));
				if (getCookie('vote')=='none') {
						$('<strong> Aun no has votado al troll!</strong><br> </h4>').appendTo($('#group_state'));	
				}else {
						$('<strong> Has votado a '+getCookie('vote')+' como troll!</strong><br> </h4>').appendTo($('#group_state'));	
				}
				if (getCookie('votedBy')=='0') {		
						$('<strong> Nadie cree que seas el troll!</strong><br> </h4>').appendTo($('#group_state'));}
				else {				
						$('<strong>'+getCookie('votedBy')+ ' personas creen que eres el troll!</strong><br> </h4>').appendTo($('#group_state'));
				}
		}).fail(function() {
...
function createGroupList(n, gid, p, s) {
	...
	if (s = 'open') {
	   ...
	   tdt.onclick = function() {		joinGroup(gid);	};	
	}
